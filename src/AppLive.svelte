<script lang="ts">
  ...
  const peak_panel_storage_key = `danmu_peak_panel:${room_id}:${live_id}`;
  const peak_threshold_storage_key = `danmu_peak_threshold:${room_id}:${live_id}`;
  ...
  function remove_all_peaks_from_ranges() {
    if (danmu_peaks.length == 0) {
      return;
    }
    const next_ranges = ranges.filter(
      (r) => !danmu_peaks.some((peak) => is_range_similar(r, peak))
    );
    if (next_ranges.length !== ranges.length) {
      ranges = next_ranges;
    }
  ...
  $: {
    if (Number.isFinite(peak_threshold)) {
      window.localStorage.setItem(
        peak_threshold_storage_key,
        String(peak_threshold)
      );
    }
  }
