...
impl WeiboRecorder {
    pub async fn new(
        ...
        log::info!("[Weibo][{}]Recorder created", room_id);

        Ok(recorder)
    }

    fn log_info(&self, message: &str) {
        log::info!("[Weibo][{}]{}", self.room_id, message);
    }

    fn log_error(&self, message: &str) {
        log::error!("[Weibo][{}]{}", self.room_id, message);
    }
    ...
    async fn check_status(&self) -> bool {
        ...
        // Weibo URLs are typically like:
        // https://weibo.com/u/5885340893 (user page)
        // https://weibo.com/l/wblive/p/show/1022:2321325026370190442592 (live page)
        let url = if self.room_id.starts_with("http") {
            self.room_id.clone()
        } else {
            format!("https://weibo.com/u/{}", self.room_id)
        };
...
#[async_trait]
impl RecorderTrait<WeiboExtra> for WeiboRecorder {
    async fn run(&self) {
